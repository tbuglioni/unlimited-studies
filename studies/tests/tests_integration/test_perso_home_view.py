...
class PersonalHome(TestCase):
    ...
    def test_add_new_book(self):
        ...
        data = {"name": "hello world", "description": "some description",
                "source_info": "some source", "users": self.user_a}
    ...
    def test_update_book(self):
        ...
        init : 4 books (book1 order:1, book2 order:2, ...)
        end : 4 books (book1 order:2, book2 order:1, ...)
        """

        self.client.login(email="john@invalid.com",
                          password="some_123_password")
        data = {"book_id": self.book_1.id,
                "name": "hello world",
        ...
        book_1_progression = UserBookMany.objects.get(
            user=self.user_a, book=self.book_1).order_book
    ...
    def test_context(self):
        ...
        self.assertEqual(
            self.response.context["recap_data"]['list_date'][1], timezone.now().strftime('%d/%m/%y'))
