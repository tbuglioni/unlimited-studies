...
class TeacherView(TestCase):
    ...
    def test_context(self):
        ...
        # book1
        self.assertEqual(self.response.context["book"], self.book_1.id)
        self.assertEqual(self.response.context["error"], 0)
        self.assertEqual(
            self.response.context["user_in_acceptation"].count(), 0)
        self.assertEqual(self.response.context["user_accepted"], [])
        ...
        self.assertEqual(
            ...
            [{"username": "lee", "lvl_avg": 5.0, "user_id": self.user_b.id}],
        )
