...
class UnsubscribeStudentBookView(TestCase):
    ...
    def test_unsubscribe_student_book_not_accepted(self):
        """ unsubscribe_student_by_owner_view : login(yes), data(chapter_id), GET"""
        self.client.login(email="john@invalid.com",
                          password="some_123_password")
        self.response = self.client.get(
            reverse("studies:unsubscribe_student", kwargs={'book': self.book_1.id, "student": self.user_b.id}))
    ...
    def test_unsubscribe_student_book_accepted(self):
        ...
        self.client.login(email="john@invalid.com",
                          password="some_123_password")
        self.response = self.client.get(
            reverse("studies:unsubscribe_student", kwargs={'book': self.book_2.id, "student": self.user_b.id}))
