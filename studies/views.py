...
def personal_home_view(request, book: int = None):
    ...
    context["new_book_from_teacher"] = UserBookMany.objects.filter(
        user=request.user, to_accept=True
    ).exists()
...
def book_view(request, book: int, chapter: int = None):
    ...
    if chapter is not None:
        ...
        paginator = Paginator(user_action.get_notes(request, chapter), 100)
    ...
    if request.POST:
        user_action.create_or_update_chapter(request, context, book)
...
def subscribe_book_view(request, book):
    ...
    UserBookMany.objects.filter(
        user=request.user, book_id=book, user_fonction="student",
        to_accept=True
    ).update(to_accept=False, order_book=book_counter)
...
def unsubscribe_book_view(request, book):
    ...
    StudiesNotesProgression.objects.filter(
        user_id=request.user.id, notes__chapter__book_id=book
    ).delete()
    books = UserBookMany.objects.filter(user=request.user, to_accept=False)
    loop = 1
    for elt in books:
...
def delete_chapter_view(request, chapter):
    ...
    chapters = Chapter.objects.filter(book=book, book__users=request.user)
    loop = 1
    for elt in chapters:
